...
<script type="text/javascript">
	...
	function onleave(btn) {
	 ...
	 function input_tuchu(btn) {
         if ($(btn).hasClass("header-w36-buton-input")) {
            ...
            if(id.length!=0){
            	$.ajax({
            		"url":"delete.draft?draft2=delete",
            		"type":"post",
            		"data":data,
            		"datatype":"json",
	...
	$(function(){
		...
		$.ajax({
			"url":"xianshi.draft?draft1=xianshi",
			"type":"post",
			"data":data,
			"datatype":"json",
			"success":function(list){
				if(list[0]=="False"){
					...
					$(".header-w36-zongji").html('<span>共'+list.length+'封,已选择<span id="xuanze">0</span>封</span>');
					for(var i=0;i<list.length;i++){
						...
						$("#tbody").append(str+list[i].reciever+"</span></td><td  style=\"cursor:pointer;\" onmouseover=\"onover(this);\" onmouseleave=\"onleave(this);\"><span>"+list[i].title+"</span></td><td><span>"+d.getFullYear()+"-"+month+"-"+d.getDate()+" "+d.getHours()+":"+d.getMinutes()+":"+d.getSeconds()+"</span></td></tr>");
					}
				}
			}
		})
